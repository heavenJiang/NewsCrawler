...
def getpageurl():
	url_list = ['https://hnwj.dahe.cn/5xw/',
	            'https://hnwj.dahe.cn/5sy/',
	            'https://hnwj.dahe.cn/5jk/',
	            'https://hnwj.dahe.cn/5ft/',
	            'https://hnwj.dahe.cn/5fy/']
	return url_list
...
def getarturl(pageurl_list, proxy, driver):
	...
	for url in pageurl_list:
		try:
			...
			results = content.find_all(class_="news-list")[:10]
...
def get_article(arturl_list, proxy, driver):
	for url in arturl_list:
		try:
			driver.get(url)
			content = BeautifulSoup(driver.page_source, "lxml").find_all(class_="xinwen")[0]
			html = content.find(class_=re.compile(r'xinwen-info\sclearfix'))
			if is_Chinese(str(html)):
				title = content.find_all(class_="xinwen-ttl")[0].h2.get_text()
				...
				if isSame(title + context) is not False:
					print("正在存储大河网“{0}”文章".format(title))
					pool.insert_one(
